...
const Divider = styled.div(() => [tw`divide-y divide-gray-700 dark:divide-gray-700`])
const Header = styled.div(() => [tw`pt-6 pb-8 space-y-2 md:space-y-5`])
const Label = styled.h2(() => [tw`text-red-400 text-xl`])

const Description = styled.p(() => [
  tw`text-xl font-serif leading-7 text-gray-500 dark:text-gray-400`,
])
...
export default function Home({ posts }) {
  return (
    <>
      ...
      {/* </Header> */}
      {/* <Divider as="ul"> */}
      <ul tw="flex flex-wrap flex-row">
        {!posts.length && 'No posts found.'}
        {posts.slice(0, MAX_DISPLAY).map((frontMatter) => {
          const { slug, date, title, summary, tags } = frontMatter
          return (
            <li key={slug}>
              <Card title={title} description={summary} href={slug} />
            </li>
